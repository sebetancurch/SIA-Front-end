...
import {
  ...
  filterSchema,
  filterSelections,
} from "@/app/(home)/faculties/list/filterFields";
import {
  DropdownMenu,
  DropdownMenuContent,
  DropdownMenuLabel,
...
export default function FacultiesTablePage() {
  ...
  useEffect(() => {
    ...
    getFaculties(request)
      .then((response: Response<ListResponse<Faculty>>) => {
        if (response.success) {
          setResponse(response.content as ListResponse<Faculty>);
        } else {
          toast({
            variant: "destructive",
    ...
    <div className="rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark">
      <div className="flex justify-between px-4 py-6 md:px-6 xl:px-7.5">
        ...
        <Sheet>
          ...
          <SheetContent className="overflow-auto">
            <SheetHeader>
              <SheetTitle>Create Faculty</SheetTitle>
              <SheetDescription>
                Be aware that the email is unique and is not registered yet in
                the app.
              </SheetDescription>
            </SheetHeader>
            <FacultyForm refresh={refresh} setRefresh={setRefresh} />
          </SheetContent>
        </Sheet>
      </div>
      ...
      {!isLoading ? (
        response?.content && response?.content.length > 0 ? (
          <>
            {response?.content.map((faculty) => (
              <Link
                ...
                key={faculty.id}
                href={"/faculties/details/" + faculty.id}
              >
                <div className="col-span-1 flex items-center">
          ...
          <EmptyList info={"Faculties not found"} />
        )
      ) : (
        <Loader />
      )}
